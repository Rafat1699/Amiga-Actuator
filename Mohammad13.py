...
async def send_initial(bus):
    ...
    for node in ACTUATOR_IDS:
        ...
        await asyncio.sleep(0.05)
    cmds = [(0x600 + n, [0x23, 0x16, 0x10, 0x01, 0xC8, 0x00, 0x01, 0x00]) for n in ACTUATOR_IDS]
...
async def controller(bus, b, a, vx, _):
    ...
    if i1 < len(xx) and s1[i1] and lookahead1 > xx[i1]:
        await send_actuator_command(bus, 22, "open")
        i1 += 1
    if i2 < len(xx) and s2[i2] and lookahead1 > xx[i2]:
        await send_actuator_command(bus, 24, "open")
        i2 += 1
    if i3 < len(xx) and s3[i3] and lookahead1 > xx[i3]:
