...
# ───────────────────────────────────────────────────────
# Load signals3.txt (now including its first column)
# ───────────────────────────────────────────────────────
def load_signal_data():
    global heading_array, c, s1, s2, s3, ds1, ds2, ds3
    path = Path(__file__).parent / "signals3.txt"
    raw = np.loadtxt(path)
    # raw.shape == (4,5): first row is [0,3.048,6.096,9.144,12.192]
    heading_array = raw[0, :]           # => [0, 3.048, 6.096, 9.144, 12.192]
    c = heading_array.copy()
    signals = raw[1:, :].astype(int)    # shape (3,5)
    s1, s2, s3 = signals                # each is length‑5
...
# ───────────────────────────────────────────────────────
# CAN setup & send helpers (unchanged)
# ───────────────────────────────────────────────────────
def setup_can_bus():
...
# ───────────────────────────────────────────────────────
# Controller (now using s1[i1], etc. to decide open/close)
# ───────────────────────────────────────────────────────
async def controller(bus, b, a, vx, _):
    ...
    if index is None:
        ...
        # serpentine logic: flip direction every other row?
        if (index + 1) % 2 == 1:
            xx = heading_array.copy()
        else:
            xx = L - heading_array

        # start with all closed, then open if initial s==1
    ...
    # now use the current s1[i1], s2[i2], s3[i3] to choose open/close
    if i1 < len(xx):
        if s1[i1] == 1 and lookahead1 > w1:
            await send_act(bus, 22, "open");  i1 += 1
        elif s1[i1] == 0 and lookahead2 > w1:
            await send_act(bus, 22, "close"); i1 += 1

    if i2 < len(xx):
        if s2[i2] == 1 and lookahead1 > w2:
            await send_act(bus, 24, "open");  i2 += 1
        elif s2[i2] == 0 and lookahead2 > w2:
            await send_act(bus, 24, "close"); i2 += 1

    if i3 < len(xx):
        if s3[i3] == 1 and lookahead1 > w3:
            await send_act(bus, 26, "open");  i3 += 1
        elif s3[i3] == 0 and lookahead2 > w3:
            await send_act(bus, 26, "close"); i3 += 1

    # final close once past end
    if b > -xx[-1]:
        for aid in ACTUATOR_IDS:
            await send_act(bus, aid, "close")
...
# ───────────────────────────────────────────────────────
# GPS streaming + controller (unchanged)
# ───────────────────────────────────────────────────────
