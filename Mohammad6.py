...
def load_signal_data():
    ...
    raw = np.loadtxt(path)
    # raw shape: (4,5)
    heading_array = raw[0, :]            # first row
    c = heading_array.copy()             # waypoint positions
    signals = raw[1:, :].astype(int)     # next 3 rows: s1, s2, s3
    s1, s2, s3 = signals
...
async def controller(bus, b, a, vx, _):
    ...
    # Compute targets and look‑aheads
    ...
    # Final close once past last waypoint
    if b > -xx[-1]:
        for aid in ACTUATOR_IDS:
            await send_act(bus, aid, "close")
